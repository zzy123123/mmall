...
public class ProductServiceImpl implements IProductService {
    ...
    public ServerResponse addOrUpdateProduct(Product product){
        if(product != null){
            ...
            if(product.getId() != null){
                ...
                if(rowCount>0) {
                    return ServerResponse.createdBySuccess("更新产品成功");
                }
                return ServerResponse.createdByErrorMessage("更新产品失败");
            }else {
                int rowCount=productMapper.insert(product);
                if(rowCount>0) {
                    return ServerResponse.createdBySuccess("新增产品成功");
                }
                return ServerResponse.createdBySuccess("新增产品失败");
            }
        }
        return ServerResponse.createdByErrorMessage("新增或更新产品参数不正确");
    }
    ...
    public ServerResponse<String> setProductSaleStatus(Integer productId,Integer status){
        ...
        if (rowCount>0){
            return ServerResponse.createdBySuccess("修改产品销售状态成功");
        }
        return ServerResponse.createdByErrorMessage("修改产品销售状态失败");
    }
    ...
    public ServerResponse<ProductDetailVo> getProductDetail(Integer productId){
        ...
        if(product == null){
            return ServerResponse.createdByErrorMessage("产品已下架或者删除");
        }
    ...
    private ProductDetailVo assembleProductDetailVo(Product product){
        ...
        if(category ==null){
            productDetailVo.setParentCategoryId(0);//设置为根节点
        }else {
    ...
    public ServerResponse<PageInfo> getProductByKeywordAndCategory(
        ...
        if(categoryId != null){
            ...
            if(category == null && StringUtils.isBlank(keyword)){
                //没有该分类且没有关键字，返回空结果集，不报错
                PageHelper.startPage(pageNum,pageSize);
                List<ProductListVo> productListVoList = Lists.newArrayList();
                PageInfo pageInfo=new PageInfo(productListVoList);
                return ServerResponse.createdBySuccess(pageInfo);
            }
            categoryIdList = iCategoryService.selectCategoryAndChildCategoryByRecursion(category.getId()).getData();
        }
